 ...
 * 计算压缩公钥
 */
function getBcecPublicKeyFromPrivateKey(privateKey) {
    let PA = G.multiply(new BigInteger(privateKey, 16));
    let x = _.leftPad(PA.getX().toBigInteger().toString(16), 64);
    let y = _.leftPad(PA.getY().toBigInteger().toString(16), 64);
    let head = 2 + (y.slice((32 * 2) - 2) & 0x01)
    return ((head < 10) ? '0' + head.toString() : head.toString()) + x;
}

/**
 * 计算04公钥
 */
function getPublicKeyFromBcecPublicKey(bcecPublicKey) {
    let PA = curve.fromBigInteger(new BigInteger(bcecPublicKey, 16));
    console.log(PA)
    return PA.toBigInteger();
}
...
module.exports = {
    ...
    doVerifySignature,
    getPoint,
    getBcecPublicKeyFromPrivateKey,
    getPublicKeyFromBcecPublicKey,
};
